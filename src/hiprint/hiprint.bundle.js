...
var languages = {}
// const ctx = require.context('./i18n', true, /\.json$/) // vite 不支持require
// ctx.keys().forEach((key) => {
// 	languages[key.match(/\.\/([^.]+)/)[1]] = ctx(key)
// })
// todo: 处理i18n国际化的json文件
const modules = import.meta.glob('./i18n/*.json', { eager: true })
Object.keys(modules).forEach((path) => {
	/* const lang = path.match(/\.\/([^.]+)/)[1] */ // 获取 ./i18n/cn.json 中的cn 不知道怎么写正则
	const lang = path.replace('.json', '').replace('./i18n/', '')
	languages[lang] = modules[path].default
})
...
var hiprint = (function (t) {
					...
					(BasePrintElement.prototype.createLineOfPosition = function (t) {
						...
						if (config.showPosition) {
							if (topPos.length) {
								...
								topPos.css(
									'top',
									this.options.posTop() - topPos.height() + 'pt'
								)
				...
				return (
					(t.prototype.init = function (e, i) {
						...
						e.forEach(function (t, e) {
							...
							(n += ' </select>')
							; (this.target = $(n)),
								i.getTarget().append(this.target),
								this.target.focus()
					}),
					(t.prototype.getValue = function () {
			...
			l = (function () {
				...
				return (
					...
					(t.prototype.beginEdit = function (t) {
						var e = this
						if (
							e.tableOptions.options.fields &&
							e.tableOptions.options.fields.length
						) {
							...
							if (
								...
								var n = e.split('#')
									; (t.title = this.title = n[0]),
										n.length > 0 && (t.columnId = t.field = this.field = n[1])
								...
								.forEach(function (column) {
									...
									switch (column.tableSummary) {
										...
										case 'min':
											...
											tableFooter.append(
												`<td style="${style}" colspan="${colspan}">${title}${min || 0
												}</td>`
											)
											break
										case 'max':
											...
											tableFooter.append(
												`<td style="${style}" colspan="${colspan}">${title}${max || 0
												}</td>`
											)
											break
										case 'text':
											tableFooter.append(
												`<td style="${style}" colspan="${colspan}">${text || ''
												}</td>`
											)
											break
										default:
							...
							t.rowColumns
								...
								.forEach(function (t, i) {
									...
									if (n.rowsColumnsMerge) {
										...
										var r = $(
											`<td style = 'display:${!(rowsColumnsArr[0] && rowsColumnsArr[1]) ? 'none' : ''
											}' rowspan = '${rowsColumnsArr[0]}' colspan = '${rowsColumnsArr[1]
											}'></td>`
										)
									} else {
										var r = $('<td></td>')
			...
			imageFit = (function () {
				...
				return (
					(t.prototype.css = function (t, e) {
						if (t && t.length) {
							if (e)
								return t.find('img').css('object-fit', e), 'object-fit:' + e
					...
					(t.prototype.createTarget = function () {
						return (
							(this.target = $(
								`<div class="hiprint-option-item hiprint-option-item-row">\n        <div class="hiprint-option-item-label">\n        ${i18n.__(
									'样式函数'
								)}\n        </div>\n        <div class="hiprint-option-item-field">\n        <textarea style="height:80px;" placeholder="function(value, options, target,templateData){/* return {color:'red'} */}" class="auto-submit"></textarea>\n        </div>\n    </div>`
							)),
							this.target
						)
					}),
												...
												$(p).hidraggable({
													...
													onStopDrag: function onStopDrag(n) {
														...
														; (s.cell.width = s.cell.width + o),
															(s.nextGrip.cell.width =
																s.nextGrip.cell.width - o),
					...
					$(this.handle).hicontextMenu({
						menus: [
							...
							},
							{
								...
								disable: function disable() {
									return (
										!t.tableCellSelector.getSingleSelect() || t.rows.length <= 1
									)
								},
								callback: function callback() {
									t.deleteRow(),
							...
							},
							{
								...
								disable: function disable() {
									return (
										!t.tableCellSelector.getSingleSelect() ||
										(t.rows.length > 0 && t.rows[0].columns.length <= 1)
									)
								},
								callback: function callback() {
									t.deleteColums(),
		...
		!(function (t) {
			...
			function o(i) {
				...
				if (
					...
					// 当前纸张宽高
					var parent = data.parent.className.endsWith('design')
						? data.parent
					...
					if (
						o.options.designTarget &&
						o.options.designTarget.options.transform
					) {
						...
						r.hidroppables.each(function () {
							...
							if (!i.hidroppable('options').disabled) {
								...
								return e.pageX > o.left &&
									e.pageX < o.left + i.outerWidth() * ptr &&
									e.pageY > o.top &&
									e.pageY < o.top + i.outerHeight() * ptr
									? (p.revert &&
										t(e.data.target).css({
											position: e.data.startPosition,
											left: e.data.startLeft,
						...
						function s(e) {
							var n = t.data(e.data.target, 'hidraggable'),
								...
								tr =
									...
									parseInt(t(i)[0].style.transform.slice(7, -1)),
											...
											if (0 != s(e)) {
												...
												0 !=
													t
														...
														.options.onBeforeDrag.call(e.data.target, e) &&
													(t(document).bind(
														'mousedown.hidraggable',
														e.data,
														i
					...
					initResizeBox: function initResizeBox(t) {
						...
						n(t).each(function () {
							...
							var r = {
								...
								sizeBox = n(
									'<div class="size-box" style="position: absolute;"></div>'
								),
							...
							n(i.options.stage)
								.on('mousemove', function (e) {
									if (d) {
										...
										var lastAngle =
											(u[0].style.transform &&
												parseInt(u[0].style.transform.slice(7, -1))) ||
											0
										...
										if (e.shiftKey) {
											...
											u.css({
												...
												height: i.numHandlerText(height),
											})
											i.options.onResize(
												e,
												i.numHandler(height),
					...
					bindHidePanel: function bindHidePanel(t) {
						if (i.maxPanelIndex < 2) {
							...
							n(e).bind('click', function (t) {
								...
								if (
									t.target.className &&
									_typeof(t.target.className) == 'string' &&
									t.target.className.includes('design')
								) {
									t.stopPropagation(),
						...
						((this.socket = window.io(this.host, {
							...
							this.socket.on('connect', function (e) {
								; (t.opened = !0),
									...
									console.log('%c云打印服务已启动','color:green;font-weight:bold'),
							...
							this.socket.on('disconnect', function () {
								console.log('%c云打印服务已断开','color:green;font-weight:bold'),
								t.opened = !1
								cb && cb(false)
							}))
	...
	/**n(33) 5200
	 * n(33)=>t[i].call(o.exports, o, o.exports, n)
	 *  t[33]为当前模块，t->o, o.exports->e, n->n
	 * @t module {i:33,l:true,exports:{/.../}}
	 * @e __webpack_exports__ t.exports 真正导出的模块
	 * @n __webpack_require__ 注册模块的函数n(*),挂载有工具函数用作定义对象等
	 */
	function (t, e, n) {
		'use strict'
		n.r(e)
		...
		var _r,
			...
			T = (function () {
				...
				return (
					...
					(t.prototype.resize = function (t, e) {
						...
						this.paperNumberTop =
							this.paperNumberTop > this.height
								? (this.paperNumberTop = parseInt(
									(this.height - 22).toString()
								))
								: this.paperNumberTop
					...
					(t.prototype.zoom = function (s) {
						if (s) {
							; (this.scale = s),
								this.target.css('transform', 'scale(' + s + ')')
							if (s > 1) {
				...
				return (
					...
					(e.prototype.initBarcode = function (designTarget, title, text) {
						...
						try {
							...
							var barcode = bwipjs.toSVG(bwipjsConfig)
							 content.html($(barcode))
							 console.log(this.options.hideTitle);
							 // 如果是undefined 则只显示text //如果是false 显示titleText+text // 如果是true 不显示
							if (!this.options.hideTitle) {
								...
								if(this.options.hideTitle===false){
									...
									content.append(
									$(
										`<div class="hiprint-printElement-barcode-content-title" style="${textStyle}">${text}</div>`
									)
								)
								}
							}
								...
								const textStyle =
									textAlign === 'justify'
										...
										if(this.options.hideTitle===false){
												...
												content.append(
									$(
										`<div class="hiprint-printElement-qrcode-content-title" style="${textStyle}">${text}</div>`
									)
								)
										}
            ...
            .hiprint-printTemplate{
              margin: -${(Number(this.panelLayoutOptions['layoutRowGap']) || 0) / 2
								}mm -${(Number(this.panelLayoutOptions['layoutColumnGap']) || 0) / 2
								}mm;
            }
              .hiprint-printTemplate .hiprint-printPanel {
                display:inline-block;
                padding: ${(Number(this.panelLayoutOptions['layoutRowGap']) || 0) / 2
								}mm ${(Number(this.panelLayoutOptions['layoutColumnGap']) || 0) / 2
								}mm;
              }
            </style>
          `
					...
					(t.prototype.clearLastPrintElement = function () {
						if (this.lastPrintElement) {
							...
							if (this.lastPrintElement._printElementOptionItems) {
								this.lastPrintElement._printElementOptionItems.forEach(
									function (t) {
										t.destroy()
									}
								)
			...
			/** hiprint.PrintTemplate(t) => template实例*/
			ct = (function () {
				function t(t) {
					var e = this
